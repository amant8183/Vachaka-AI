    ...
    io.on("connection", (socket: Socket) => {
        ...
        socket.on("voice_input", async (data: { conversationId: string; audioBuffer: Buffer }) => {
            try {
                ...
                console.log(`⏱️ [PERF] STT: ${sttDuration}ms | Text: "${transcript.substring(0, 50)}..."`);
                ...
                console.log(`⏱️ [PERF] AI: ${aiDuration}ms | Response: ${fullResponse.length} chars`);
                const aiMessage = await memoryService.addMessage(
                    conversationId,
                    "assistant",
                    fullResponse
                ...
                if (ttsService.isAvailable()) {
                    try {
                        ...
                        console.log(`⏱️ [PERF] STT=${sttDuration}ms | AI=${aiDuration}ms | TTS=${ttsDuration}ms\n`);
                        io.to(conversationId).emit("ai_voice_response", {
                            audio: audioBase64,
                            mimeType: mimeType,
                        });
